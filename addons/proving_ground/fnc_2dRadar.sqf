...
- show tents, helicrashes, players, vehicles,nearby animals and zombies
- nice icons 
...
if (isNil "tpe") then {tpe = false;};		//Use built-in teleport
if (isNil "tpa") then {tpa = true;};		//Use alternative teleport (could be used to tp on buildings)(not works for vehicles)(credits to monky)
if (isNil "uvb") then {uvb = true;};		//Use bypass of current antiteleport script (ViLayer)
...
ptxt = "You";  					//Text to identify You on map/gps. Leave blank ("") if You want to see only black icon
...
moptions = 
{
	...
	[
		...
		[format["Show tents: %1",sht], [5], "", -5, [["expression", "sht = !sht;"]], "1", "1"],
		[format["Show nearby zombies: %1",shz], [6], "", -5, [["expression", "shz = !shz;"]], "1", "1"],
		[format["Show nearby animals: %1",sha], [7], "", -5, [["expression", "sha = !sha;"]], "1", "1"],
		[format["Use alternative teleport: %1",tpa], [8], "", -5, [["expression", "tpa = !tpa;"]], "1", "1"],
		[format["Use 'anticheat' bypass: %1",uvb], [9], "", -5, [["expression", "uvb = !uvb;"]], "1", "1"]
	];  
...
drawic = 
{
	if (!isnil "mapm") then 
	{
		...
		{
			if ((shz) and (!isnull _x)) then {_ctrl drawIcon ["\ca\ui\data\iconman_ca.paa", zmc, getPosASL _x, _iscale*30, _iscale*30, getDir _x, "", 1];};
		} foreach zombies;
		{
			if ((sha) and (!isnull _x)) then {_ctrl drawIcon ["\ca\ui\data\icon_animals_ca.paa", amc, getPosASL _x, _iscale*15, _iscale*15, 0, "", 1];};
		} foreach animalsl;

	
		{
			if  (!isnull _x) then {_ctrl drawIcon ["\ca\air2\data\ui\icon_uh1y_ca.paa", wmc, getPosASL _x, 30, 30, 0, "", 1];};
		} foreach helicrashes;
	
		{
			if ((!isnull _x) and (sht)) then {_ctrl drawIcon ["\ca\ui\data\markers\waypoint.paa", tmc, getPosASL _x, 30, 30, 0, "", 1];};
		} foreach tents;
...
{	
 	...
 	if (!isnil "mapm") then 
	{
		...
		//titleText [format ["%1,%2,%3,%4,%5,%6,%7",(diag_tickTime - start_time), count vlist, count plist, count clist,count slist],"PLAIN DOWN"];titleFadeOut 2;

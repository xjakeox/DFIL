...
if (INV_PLAYERLIST select _transfer == player) exitWith 
	...
	if (_Mishy < _transfermoney) then 
		...
		bnkgeld = bnkgeld + _transfermoney;
		['Mishy', -(_transfermoney)] call INV_AddInvItem;
...
if (bnkgeld < _steuern) then 
	...
	bnkgeld = bnkgeld - _steuern;
	INV_SteuernGezahlt = (INV_SteuernGezahlt + (_steuern - _transfermoney));
	(format ["if (INV_ROLESTRING == ""%1"") then {bnkgeld = bnkgeld + %2; player groupChat format[localize ""STRS_bank_account_getmoney"", %2, %3];};", _targetString, (_transfermoney call ISSE_str_IntToStr), player]) call broadcast;
...
if (bnkgeld >= _withdraw) then 

	{

	bnkgeld = bnkgeld - _withdraw;
	['Mishy', (_withdraw)] call INV_AddInvItem;

...
if (_art == "spielerliste") then 
{
	...
	lbAdd [1, format ["Money: $%2", localize "STRS_statdialog_health", (('Mishy' call INV_GetItemAmount) call ISSE_str_IntToStr)]];
	lbAdd [1, format ["Savings: $%2", localize "STRS_statdialog_health", bnkgeld]];
	lbAdd [1, (format ["Minutes Alive:  %1", minsalive])];
	lbAdd [1, (format ["Hours Alive:  %1", hoursalive])];
	...
	if(_control4 == _gangname)then{if(_territory == "None")then{_territory = "Old Military Base"}else{_territory = _territory + ", Old Military Base"};};	
	...
	lbAdd [1, "THE GOVERNOR IS NOT ABOVE THE LAW"];
	...
	lbAdd [1, "Hacker Logs:"];
	if (isnil "CheatInfo") then {lbAdd [1, "No Hackers have logged on in this session."]} else {lbAdd [1, (format ["%1", CheatInfo])];};		
};

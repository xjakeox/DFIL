...
while {true} do 
	...
	while {true} do 
		...
		if (player != vehicle player and vehicle player distance _ziel < 30 and speed vehicle player < 2 and workplacejob_taxi_kundeactive and INV_LocalTaxiKunde in vehicle player) exitWith 
			...
			_Mishy = ((500 max(round((((_spielerstart distance _start)+(_start distance _ziel))*workplacejob_taxi_multiplikator)-(time-_taxizeit))))min workplacejob_taxi_maxmoney);
			if (_Mishy < 0) then {_Mishy = 0};
			['Mishy', _Mishy] call INV_AddInvItem;
			player groupChat format [localize "STRS_workplacemission_taxi_success",_Mishy];
			format["%1 action [""getOut"", (%2)]; unassignVehicle %1;",INV_LocalTaxiKunde,(vehicle player)] call broadcast;
			sleep 5;

...
if (_Mishy <= huren_cost) exitWith {role groupChat localize "STRS_huren_nomoney"};
if (slavescriptrunning == 1) exitwith {player groupchat "script already running"};
slavescriptrunning = 1;		
['Mishy', -(huren_cost)] call INV_AddInvItem;
...
while {true} do 
	...
	if (_exitvar == 1) exitWith {};
		...
		if ((player distance (_x select 0)) < (_x select 1)) then {call compile format ["arbeiterMishy%1 = arbeiterMishy%1 + hoemoneyprosekunde + round(random 500 - random 500);", _arbeiternummer];};
	...
	for [{_i=0}, {_i < 120}, {_i=_i+1}] do 
		...
		if ((player distance _hurenname <= 5) and ((player distance brothel1a < 15) or (player distance brothel2a < 15) or (player distance brothel3a < 15))) then {
			...
			call compile format ["arbeiterMishy%1 = 0;", _arbeiternummer];	
			player groupChat format [localize "STRS_huren_Mishyabnahme_ja", (_arbeiterMishy call ISSE_str_IntToStr)];
...
if (_arbeiterMishy > 0) then 
	...
	player groupChat localize "STRS_huren_Mishyabnahme_nein";

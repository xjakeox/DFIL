...
if (Antwort == 1) then
	...
	if ((_Mishy + bnkMishy) < _strafe) exitwith 
	...
	if (bnkMishy > _strafe) exitwith {bnkMishy = bnkMishy - _strafe;};
...
if (["INV_InventarArray", "drug"] call INV_StorageHasKindOf) then 
	...
	(format ["if(!(""trafficking drugs"" in %1_reason))then{%1_reason = %1_reason + [""trafficking drugs""]}; %1_wanted = 1; kopfMishy_%1 = kopfMishy_%1 + (%3); if (player == %2) then {player groupChat format[localize ""STRS_civmenu_hasdrugs"", %1, %4, (%3/2)];}; titletext [format[localize ""STRS_civmenucheck_haddrugs"", %1, %3], ""plain""];", player, _checkcop, drugsvalue, ceil(drugsvalue/25000)]) call broadcast;	
...
if ((_loopart == "arrest") and (player distance prisonflag <= 50))  then 
...
if (_civkopfMishy != 0) then 
	...
	(format['if(rolestring == "%1")then{bnkMishy = bnkMishy + %2; player groupChat "This civ had a bounty of $%2! You got that bounty!"}', _copobj, _civkopfMishy]) call broadcast;
...
["Mishy", -(_amounttosteal)] call INV_AddInvItem;

(format ['if (rolestring == "%1") then {["Mishy", %2] call INV_AddInvItem;};hint "%1 stole %2 from %3";',_aktionsStarter, _amounttosteal, player]) call broadcast;

...
if (iscop) then 
	...
	if(_killedbyvehicle)then
		...
		INV_LizenzOwner = INV_LizenzOwner - ["Pistollicense","bomb","assassinlic","riflelicense"];
		["INV_LizenzOwner", INV_LizenzOwner] spawn ClientSaveVar;
		execVM "Scripts\statSave\jaysantiglitch.sqf";
...
if (_killedbyvehicle) then 
	{
	(format ["[""KilledCiv"", %1] spawn Isse_AddCrimeLogEntry; if(%1 in civarray)then{if(!(""Hit and Run"" in %1_reason))then{%1_reason = %1_reason + [""Hit and Run""]}; kopfMishy_%1 = kopfMishy_%1 + 30000};", player]) call broadcast;
	} 
	else 
	{
...
if (_killedbyvehicle) then 
	{
	(format ["[""KilledCop"", %1, ""%2""] spawn Isse_AddCrimeLogEntry; if(%1 in civarray)then{if(!(""Hit and run"" in %1_reason))then{%1_reason = %1_reason + [""Hit and run""]}; kopfMishy_%1 = kopfMishy_%1 + 30000};", player,  _killer]) call broadcast;
	}
	else
	{

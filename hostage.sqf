...
(format['if(!("Assassin" in %1_reason))then{%1_reason = %1_reason + ["Hostage Taker"]}; %1_wanted = 1; kopfMishy_%1 = kopfMishy_%1 + 150000;', player]) call broadcast;
...
while {true} do 
	...
	*/
	...
	if (_minutecounter >= 1200 and alive player) exitWith
	
		{
		bnkgeld = bnkgeld + 200000;
		player groupchat "Well done, you kept the hostage for 20 minutes, $200000 has been transfered to your account.";
		sleep 10;
		"server globalchat ""Hostage Taker WINS, he kept the hostage for 20 minutes."";" call broadcast;
		sleep 1;
		deletevehicle hostage1;
		deletemarker "htargetmarker";
		_minutecounter = 0;
	...
	if (!alive hostage1 and alive player) exitWith 
		...
		//(format['if(!("Assassinated VIP" in %1_reason))then{%1_reason = %1_reason + ["Assassinated VIP"]}; %1_wanted = 1; kopfMishy_%1 = kopfMishy_%1 + 25000;', player]) call broadcast;
		deletevehicle hostage1;
		deletemarker "htargetmarker";
	...
	if(hostage1 distance getmarkerpos "hostagezone" > 125) exitwith
		...
		_copbonus = 60000;
		if (iscop) then {bnkgeld = (bnkgeld + _copbonus); player sidechat format[""you received $%1 for hostage taker fleeing the area"", _copbonus];};
		" call broadcast;
		sleep 2;
	...
	if(player distance hostage1 > 200) exitwith
		...
		_copplayernumber = playersNumber west;
		_copbonus = 60000;
		if (iscop) then {bnkgeld = (bnkgeld + _copbonus); player sidechat format[""you received $%1 for the successful rescue of the hostage"", _copbonus];};
		" call broadcast;
		sleep 2;

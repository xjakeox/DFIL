...
_type   = typeof _car;
	...
	if ((player == driver _car) and (_car distance _x < 40) and (_speed >= 91) and !(_car iskindof "Air")) then 
		...
		player groupchat format["You have been set wanted for speeding! Your speed: %1", round _speed];
		sleep 2;
		
		(format['if(!("Breaking the speed limit" in %1_reason))then{%1_reason = %1_reason + ["Breaking the speed limit"]}; %1_wanted = 1; kopfMishy_%1 = kopfMishy_%1 + 2500;', player]) call broadcast;

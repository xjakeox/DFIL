...
(format['if(!("Assassin" in %1_reason))then{%1_reason = %1_reason + ["Assassin"]}; %1_wanted = 1; kopfMishy_%1 = kopfMishy_%1 + 50000;', player]) call broadcast;
...
while {true} do 
	...
	*/
	...
	if(VIPtarget distance getmarkerpos "policebase" < 100) exitwith
		...
		_copbonus = 40000;
		if (iscop) then {bnkgeld = (bnkgeld + _copbonus); player sidechat format[""you received $%1 for the successful rescue of the VIP"", _copbonus];};
		" call broadcast;
		sleep 2;

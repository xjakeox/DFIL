...
if (_Mishy <= slave_cost) exitWith {role groupChat localize "STRS_slave_nomoney"};
if (slavescriptrunning == 1) exitwith {player groupchat "script already running"};
slavescriptrunning = 1;		
['Mishy', -(slave_cost)] call INV_AddInvItem;
...
call compile format ["arbeiterMishy%1 = 0;", _arbeiternummer];
...
while {true} do 
	...
	if (_exitvar == 1) exitWith {};
		...
		if ((player distance (_x select 0)) < (_x select 1)) then {call compile format ["arbeiterMishy%1 = arbeiterMishy%1 + slavemoneyprosekunde + round(random 500 - random 500);", _arbeiternummer];};
	...
	for [{_i=0}, {_i < 60}, {_i=_i+1}] do 
		...
		if ((player distance _slavename >= 5 or (call compile format ["arbeiterMishy%1 == 0", _arbeiternummer])) and _a1 == 1) then {player removeaction _action; _a1 = 0;};																																													
			
		if (player distance _slavename < 5 and _a1 == 0 and (call compile format ["arbeiterMishy%1 > 0", _arbeiternummer])) then {_action = player addaction [localize "STRS_slave_moneyaction", "slaves.sqf",["Mishy", _arbeiternummer]]; _a1 = 1;};															
...
if (_art == "Mishy") exitWith 

{

_arbeiternummer = (_this select 3) select 1;
_arbeiterMishy   = call compile format ["arbeiterMishy%1;", _arbeiternummer];
								

if (_arbeiterMishy > 0) then 

	{

	['Mishy', _arbeiterMishy] call INV_AddInvItem;
	call compile format ["arbeiterMishy%1 = 0;", _arbeiternummer];	
	player groupChat format [localize "STRS_slave_Mishyabnahme_ja", (_arbeiterMishy call ISSE_str_IntToStr)];

	} 

else    {	

	player groupChat localize "STRS_slave_Mishyabnahme_nein";

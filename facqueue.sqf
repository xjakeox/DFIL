if(typename _this == "ARRAY") exitwith
	...
	if(_Mishy < facworkercost)exitwith{player groupchat "you do not have enough money"};
	
	["Mishy", -facworkercost] call INV_AddInvItem;

	call compile format['%1workers = %1workers + 1;["%1workers", %1workers] spawn ClientSaveVar;', _queue];

	player groupchat "1 Factory worker hired!";

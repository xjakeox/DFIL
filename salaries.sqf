...
sleep 60;
player groupChat format[localize "STRS_Mishy_countdown", "2"];
sleep 60;
player groupChat format[localize "STRS_Mishy_countdown", "1"];
sleep 60;
...
bnkgeld = bnkgeld + (round _income);
player groupChat format[localize "STRS_Mishy_copmoneyadd", rolestring, ((round _income) call ISSE_str_IntToStr)];
sleep 1;					
...
if ((alive player) and (deadcam == 0)) then 
	...
	_uniPaycheck       = 0;	
	_unimsg            = ""; 	
	_atworkplacemsg    = localize "STRS_Mishy_nowere";									
	_hashideoutmsg     = "";
...
/*
		...
		_atworkplacemsg     = localize "STRS_Mishy_were";	
...
*/		
	...
	bnkgeld = bnkgeld + _income;	
	player groupChat format[localize "STRS_Mishy_civmoneyadd", rolestring, (_income call ISSE_str_IntToStr)];		
	...
	if (isMayor) then 
		...
		bnkgeld = bnkgeld + MayorSteuern;                           																					
		bnkgeld = bnkgeld + MayorExtraPay;                          																											
	...
	player groupChat format[localize "STRS_Mishy_paycheckdead"];

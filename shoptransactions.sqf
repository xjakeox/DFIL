...
if(_Mishy == 0)exitwith{player groupchat localize "STRS_inv_buyitems_keinMishy"; _exitvar = 1};
...
if (_itemart == "Item" and instock) then 
	...
	if (_Mishy < _cost) then 

		{
												
		_menge = (floor(_Mishy/_CostMitTax));									
		_cost  = _menge*_CostMitTax;																														
		if (_menge <= 0) exitWith {player groupChat localize "STRS_inv_buyitems_keinMishy"; _exitvar = 1;};
	...
	if (_infos call INV_getitemIsIllegal and _cost > 0 and _infos call INV_getitemKindOf == "drug") then
		...
		for "_c" from 0 to (count gangsarray - 1) do 
			...
			if(_control == _gang and count _members > 0)then
				...
				format['if(name player in %1)then{player groupchat "You received $%2 from a drug sale"; bnkgeld = bnkgeld + %2};', _members, _income] call broadcast;
...
if (_itemart == "fahrzeug" and instock) then 
	...
	if (_Mishy < _CostMitTax) exitWith {player groupChat localize "STRS_inv_buyitems_keinMishy"; _exitvar = 1};																		

...
_housename = _house select 0;
	...
	(format['if(!("Burglary" in %1_reason))then{%1_reason = %1_reason + ["Burglary"]}; %1_wanted = 1;kopfMishy_%1 = kopfMishy_%1 + 7000;', player]) call broadcast;
	...
	(format ["%1 switchmove ""%2"";", player, ""]) call broadcast;
	format["hint ""%1 was seen attempting to break into %2 house!"";", (name player), _marker] call broadcast;
	(format['if(!("AttemptedBurglary" in %1_reason))then{%1_reason = %1_reason + ["Attempted Burglary"]}; %1_wanted = 1;kopfMishy_%1 = kopfMishy_%1 + 5000;', player]) call broadcast;
	};
	[_item, -1] call INV_AddInvItem;
};

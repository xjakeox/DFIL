...

{
	...
	if(INV_shortcuts)then{INV_shortcuts=false; titletext["IL keys off", "PLAIN DOWN"];[] execVM "actions\actionsRemove.sqs"}else{INV_shortcuts=true; titletext["IL keys on", "PLAIN DOWN"];[] execVM "actions\govactions.sqf"};};
	//L key
	case 38: 
	...
	if(isEngineOn _vcl and !isnil "_nos") then
		...
		_spd  = speed _vcl;
		_fuel = fuel _vcl;
		_vcl setVelocity [(_vel select 0) * 1.02, (_vel select 1) * 1.02, (_vel select 2) * 0.99];
		_vcl setfuel (_fuel - 0.0002);
